    ...
    public class PrintPageViewModel : INotifyPropertyChanged
    {
        ...
        void Decode(string words)
        {
           ...
           // if (Ttripodo >= 10000) Ttripodo = (
            ...
            if (check == "H")
            {
                Tinstant = (Convert.ToDecimal(words.Remove(0, 1))) / (decimal)100;
                if (Tspeed >= 1)
                {
                    Tinstant1 = (Convert.ToDecimal(words.Remove(0, 1))) / (decimal)100;
                    Print1Page.tick = 1;
                }
                else Tinstant1 = 0;
            }
            ...
            if (check == "I")
            {
                Ttrip100 = (Convert.ToDecimal(words.Remove(0, 1))) / (decimal)100;
                Ttrip1001 = (Convert.ToDecimal(words.Remove(0, 1))) / (decimal)100;
            }
            if (check == "J") Ttank100 = (Convert.ToDecimal(words.Remove(0, 1))) / (decimal)100;
            if (check == "L") Ttripfuel = (Convert.ToDecimal(words.Remove(0, 1))) / (decimal)100;
        ...
        void ResetTank(IPeripheral selectedPeripheral)
        {
            ...
            { Endd = true;
                ...
                if (action == "Partial")
                {
                    ...
                    Device.BeginInvokeOnMainThread(async () =>
                    {
                        TextToPrint = await App.Current.MainPage.DisplayPromptAsync("Partial Refuel", "How much fuel have you added to the tank *1000? (20.69L = 20,690)", "Save", "Cancel", "0", 8, keyboard: Keyboard.Numeric, "0");
